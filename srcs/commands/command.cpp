...
/*   Updated: 2024/06/03 17:47:07 by abelfany         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    if(getClientitoByIndex(i-1).isAuthed() == false) {
        ...
        else if (args[0] == "invite")
        {
            ...
            try {
                ...
                if(obj.checkUserexist(args[1]) == true)
                    sendMsgToClient(getClientitoByIndex(i-1).getClinetFd(), ERR_USERONCHANNEL(host, nick, obj.getChannelNameDisplay()));
                else if(obj.getPrvBynickname(nick) == false)
                    sendMsgToClient(getClientitoByIndex(i-1).getClinetFd(), ERR_CHANOPRIVSNEEDED(nick, obj.getChannelNameDisplay()));
                else {
                    if(args.size() < 3) {
                        sendMsgToClient(getClientitoByIndex(i-1).getClinetFd(), ERR_NEEDMOREPARAMS(nick, args[2]));
                        return ;
                    }
                    if (!alreadyAmember(getClientitoByIndex(i-1).getClinetFd(), obj)) {
                        sendMsgToClient(getClientitoByIndex(i-1).getClinetFd(), ERR_NOTONCHANNEL(nick, args[2]));
                        return ;
                    }
            ...
            {
                sendMsgToClient(getClientitoByIndex(i-1).getClinetFd(), ERR_NOSUCHCHANNEL(host, nick, args[2]));
                args.clear();
                return ;
            }
        }
